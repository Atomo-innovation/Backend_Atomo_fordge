...
class FrameReader:
    """
    Connects to an RTSP source with patient retry logic.

    MediaMTX creates the path on-demand: the upstream camera connection can
    take 5-15 s to stabilise after /api/cameras registers it.  We must NOT
    call sys.exit() on the first failed cap.isOpened() — instead we keep
    retrying in a background thread and let the main loop wait up to
    OPEN_TIMEOUT seconds for the first real frame.
    """
    OPEN_TIMEOUT  = 60     # seconds to wait for first frame before giving up
    OPEN_RETRY_S  = 3      # seconds between VideoCapture() re-attempts
    READ_FAIL_MAX = 20     # consecutive read failures before full reconnect
    RECONNECT_S   = 3      # seconds to wait before reconnecting
    ...
    def __init__(self, source):
        ...
        self._ready = threading.Event()   # set when first frame arrives

    def _make_cap(self):
        cap = cv.VideoCapture(self.source, cv.CAP_FFMPEG)
        try: cap.set(cv.CAP_PROP_THREAD_COUNT, 1)
        except: pass
        cap.set(cv.CAP_PROP_BUFFERSIZE, 2)
        return cap

    def start(self):
        # Spin the reader thread immediately — it will keep retrying open.
        self.running = True
        threading.Thread(target=self._loop, daemon=True).start()
        return self   # always succeeds; caller waits on wait_ready()

    def wait_ready(self, timeout=None):
        """Block until the first frame arrives (or timeout).  Returns True on success."""
        return self._ready.wait(timeout=timeout or self.OPEN_TIMEOUT)
    ...
    def _loop(self):
        ...
        while self.running:
            if not cap.isOpened():
                # Path not ready yet — keep retrying quietly
                cap.release()
            ...
            if not ret or frame is None or frame.size == 0:
                fail += 1
                if fail >= self.READ_FAIL_MAX:
                    # Stream dropped — full reconnect
                    cap.release()
            ...
            # Latch dimensions on first good frame
            if self.width == 0:
            ...
            self._ready.set()   # unblocks wait_ready()
...
if __name__ == '__main__':
    ...
    reader.start()   # always succeeds — spawns retry thread

    if args.json_stream:
        emit({'type': 'info', 'msg': f'Waiting for RTSP stream (up to {FrameReader.OPEN_TIMEOUT}s)…'})
    else:
        print(f'[Fire] waiting for stream — up to {FrameReader.OPEN_TIMEOUT}s…', file=sys.stderr)

    if not reader.wait_ready(timeout=FrameReader.OPEN_TIMEOUT):
        sys.exit('[ERROR] Stream timeout — MediaMTX path never delivered a frame')
