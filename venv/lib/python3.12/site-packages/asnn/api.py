import os,sys
import ctypes
from asnn.aplatform.electron.electron_nn_api import electron
from numpy import *
...
class asnn:
    ...
    Class asnn is the control interface for Neural Network, 
    
    all NPU-related functions and operations are included in this class.

    Attributes: 
        board: Board model. Board model list: Electron
    '''

    def __init__ (self, board=None):

        if board == None:
            print('default board is Electron')
            self.board = asnn_board.BOARD_Electron
        elif board == 'Electron' or 'VIM3L':
            self.board = asnn_board.BOARD_Electron
        else :
    ...
    def nn_init (self, library=None, model=None, level=0):
        ...
        Args:
            library: (Only valid for Electron) The path for your C static librarys
            model:  The path for your model file.(Electron is nbg file)
        ...
        if self.board == asnn_board.BOARD_Electron:
            ...
            self.asnn_api = electron()
            val = self.asnn_api.neural_network_create(library, model, level)
            if val == 0 :
                return asnn_stat.STAT_SUCCESS
            else :
                return asnn_stat.STAT_FAIL
        sys.exit('Unsupported board !!!')
    ...
    def nn_set_inputs (self, img, platform=None, reorder='0 1 2', tensor=1):
        ...
        if self.board == asnn_board.BOARD_Electron:
            for i in range(0,tensor):
                cv_img = img[i]
                input_tensor = self.asnn_api.neural_network_get_input_tensor(i)
                if reorder == '0 1 2':
                    reorder_format = 0
                ...
                if input_tensor.dtype.nn_type == 0x002 :
                    ...
                    val = self.asnn_api.neural_network_set_input_i8(pixel_data, reorder_format, platform_format, i)
                if input_tensor.dtype.nn_type == 0x003 :
                    pixel_data = cv_img.flatten()
                    pixel_data = pixel_data.ctypes.data_as(ctypes. POINTER(ctypes.c_float))
                    val = self.asnn_api.neural_network_set_input_u8(pixel_data, reorder_format, platform_format, i)
                if input_tensor.dtype.nn_type == 0x004 :
                    pixel_data = cv_img.flatten()
                    pixel_data = pixel_data.ctypes.data_as(ctypes. POINTER(ctypes.c_float))
                    val = self.asnn_api.neural_network_set_input_i16(pixel_data, reorder_format, platform_format, i)
                if input_tensor.dtype.nn_type == 0x00f :
                    ...
                    val = self.asnn_api.neural_network_set_input_f16(pixel_data, reorder_format, platform_format, i)
    ...
    def nn_run ( self ):
        ...
        if self.board == asnn_board.BOARD_Electron:
            val = self.asnn_api.neural_network_process()
            if val == 0 :
                return asnn_stat.STAT_SUCCESS
            else :
                return asnn_stat.STAT_FAIL
        ...
        if self.board == asnn_board.BOARD_Electron:
            for i in range(tensor) :
                if output_format == output_format.OUT_FORMAT_UINT8 :
                    val = self.asnn_api.neural_network_get_convert_tensor_data_format_u8(i)
                elif output_format == output_format.OUT_FORMAT_INT8 :
                    val = self.asnn_api.neural_network_get_convert_tensor_data_format_i8(i)
                elif output_format == output_format.OUT_FORMAT_INT16 :
                    val  = self.asnn_api.neural_network_get_convert_tensor_data_format_i16(i)
                elif output_format == output_format.OUT_FORMAT_FLOAT32 :
                    val = self.asnn_api.neural_network_get_convert_tensor_data_format_f32(i)
                else :
    ...
    def nn_inference (self, cv_img, platform=None, reorder='0 1 2', input_tensor=1,output_tensor=1, output_format=output_format.OUT_FORMAT_FLOAT32):
        ...
        if self.board == asnn_board.BOARD_Electron:
            ...
            if res == asnn_stat.STAT_FAIL :
                sys.exit('Set nn inputs error !!!')

            res = self.nn_run()
            if res != asnn_stat.STAT_SUCCESS :
    ...
    def nn_get_output_tensor_info (self, num):
        ...
        if self.board == asnn_board.BOARD_Electron:
            return self.asnn_api.neural_network_get_output_tensor(num)
        else :
            sys.exit('Unsupported board !!!')
